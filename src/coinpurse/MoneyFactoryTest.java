...
public class MoneyFactoryTest {
	...
	private MoneyFactory thFactory;
	...
	public void setUp() {
		...
		thFactory = MoneyFactory.getInstance();
	}
	...
	public void testInsert() {
		...
		assertTrue(purse.insert(thFactory.createMoney(5)));
		assertTrue(purse.insert(thFactory.createMoney(10)));
		assertTrue(purse.insert(thFactory.createMoney(1)));
		assertEquals(3, purse.count());
		// purse is full so insert should fail
		assertFalse(purse.insert(thFactory.createMoney(1)));
	}
	...
	public void testIsFull() { // borderline case (capacity 1)
		...
		purse.insert(thFactory.createMoney(1));
		...
		for (int k = 1; k <= capacity; k++) {
			assertFalse(purse.isFull());
			purse.insert(thFactory.createMoney(5));
		}
		// should be full now
		assertTrue(purse.isFull());
		assertFalse(purse.insert(thFactory.createMoney(5)));
	}
	...
	public void testInsertSameCoin() {
		...
		assertTrue(purse.insert(thFactory.createMoney(value)));
		assertTrue(purse.insert(thFactory.createMoney(value))); // should be allowed
		assertTrue(purse.insert(thFactory.createMoney(value))); // should be allowed
		assertTrue(purse.insert(thFactory.createMoney(value))); // should be allowed
		assertTrue(purse.insert(thFactory.createMoney(value))); // should be allowed
	...
	public void testEasyWithdraw() {
		...
		for (double value : values) {
			Valuable valuable = thFactory.createMoney(value);
	...
	public void testMultiWithdraw() {
		Purse purse = new Purse(10);
		Valuable[] valuable = { thFactory.createMoney(5), thFactory.createMoney(10.0), thFactory.createMoney(1.0), thFactory.createMoney(5.0) };
		// insert them all
	...
	public void testWithdrawEverything() {
		...
		List<Valuable> valuable = Arrays.asList(thFactory.createMoney(1.0), thFactory.createMoney(0.5), thFactory.createMoney(10.0),
				thFactory.createMoney(0.25), thFactory.createMoney(5.0));
	...
	public void testImpossibleWithdraw() {
		...
		purse.insert(thFactory.createMoney(20));
		assertNull(purse.withdraw(1));
		assertNull(purse.withdraw(19));
		assertNull(purse.withdraw(21));
		purse.insert(thFactory.createMoney(20)); // now it has 20 + 20
	...
	public void testSerialNumber() {
		Valuable v1 = thFactory.createMoney(100);
		Valuable v2 = thFactory.createMoney(100);
		Valuable v3 = thFactory.createMoney(50);
		Valuable v4 = thFactory.createMoney(50);
		Valuable v5 = thFactory.createMoney(20);
